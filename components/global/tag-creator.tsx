...
import { PlusCircleIcon, X, TrashIcon } from "lucide-react";
...
const TagCreator = ({ subAccountId, getSelectedTags, defaultTags }: Props) => {
  ...
  useEffect(() => {
    if (subAccountId) {
      const fetchData = async () => {
        const rsp = await getTagsForSubaccount(subAccountId);
        if (rsp) {
          setTags(rsp.Tags);
        }
      };
  ...
  const handleAddTag = async () => {
    ...
    setSelectedColor("");
    try {
      const rsp = await upsertTag(subAccountId, tagData);
      toast({
        title: "Created the tag",
      });
      await saveActivityLogsNotification({
        agencyId: undefined,
        description: `Updated a tag | ${rsp?.name}`,
        subaccountId: subAccountId,
      });
    } catch (error) {
      toast({
